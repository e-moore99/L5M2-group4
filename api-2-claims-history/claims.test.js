...
describe("claims keyword detection API", () => {
  ...
  it("should return 'Input must be text only.' if there are numbers or special characters", async () => {
    const response = await request(app).post("/endpoint-goes-here⚠️").send({ text: "Been in 1 crash back in March/" }); // could be split into two tests, one for a number and another for special characters

    expect(response.status).toBe(400);
    expect(response.body.message).toBe("Input must be text only.");
  });

  // invalid: no numbers or special characters within otherwise valid keywords
  it("should return 'Input must be text only.' if there are numbers or special characters", async () => {
    const response = await request(app)
      .post("/endpoint-goes-here⚠️")
      .send({ text: "Had a c0llide event last week, done scratch,ed up my car." });
